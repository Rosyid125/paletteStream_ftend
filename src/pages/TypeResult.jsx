...
import { Shapes, Heart, MessageCircle, Bookmark, Loader2, MoreHorizontal, Edit, Trash2, Flag } from "lucide-react"; // Using Shapes as a generic type icon
...
// --- Constants ---
const RESULTS_PAGE_LIMIT = 9; // Default limit (adjust if API differs)
...
export default function TypeResult() {
  ...
  const typeQuery = searchParams.get("query") || ""; // Get the type query
  ...
  const fetchResults = useCallback(
    async (pageNum, typeValue) => {
      // Don't fetch if type is empty
      if (!typeValue) {
        setResults([]);
        setHasMore(false);
      ...
      console.log(`Fetching type results page: ${pageNum}, type: ${typeValue}`);

      try {
        const params = {
          query: typeValue, // Use the type value for the query param
          page: pageNum,
          limit: limit,
          viewerId: CURRENT_USER_ID ?? 0,
        };
        // --- Use the correct endpoint ---
        const response = await api.get("/posts/type", { params });
  ...
  // --- Effect for Initial Load and Type Query Change ---
  useEffect(() => {
    // Reset state when the typeQuery changes
    setResults([]);
    setCurrentPage(1); // Reset to page 1 for new type
    setHasMore(true);
    setInitialLoadComplete(false);
    setLoadingMore(false);
    setLoading(true); // Start loading for the new type
    // Fetch page 1 with the current type from the URL
    fetchResults(1, typeQuery);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [typeQuery]); // Re-run ONLY when the 'typeQuery' from the URL changes
    ...
    (node) => {
      ...
      observer.current = new IntersectionObserver((entries) => {
        if (entries[0].isIntersecting && hasMore) {
          console.log("Intersection observer triggered fetch for next page (type)");
          // Fetch the *next* page using the current type
          fetchResults(currentPage, typeQuery);
        }
      });
      if (node) observer.current.observe(node);
    },
    [loading, loadingMore, hasMore, fetchResults, currentPage, typeQuery] // Dependencies
  );
  ...
  return (
    <div className="container mx-auto space-y-6 p-4 md:p-6">
        ...
        <CardHeader>
          ...
          <CardDescription>
            ...
            {typeQuery ? (
              ...
              <span className="ml-1 italic text-muted-foreground"> No type specified in URL.</span>
            )}
          </CardDescription>
        </CardHeader>
      </Card>
      ...
      <div className="mt-6">
        ...
        {loading && <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3"> {Array.from({ length: limit }).map((_, index) => renderSkeleton(`initial-skeleton-${index}`))} </div>} {/* Displayed Results Grid */}
        ...
        {initialLoadComplete && !loading && results.length === 0 && !error && (
          <div className="text-center text-muted-foreground py-10 col-span-full">
            <p>No posts found for the type "{typeQuery}".</p>
